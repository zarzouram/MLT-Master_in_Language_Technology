...
def display_tree(tree, filepath="", printflag=0):
    # return display(Markdown(f"<img src=\"data:image/png;base64,{_repr_png_(tree)}\">"))
    import base64
    thetree = _repr_png_(tree)
    display(Markdown(f"<img src=\"data:image/png;base64,{thetree}\">"))
    if printflag:
        thetree_bytes = thetree.encode()
        with open(filepath, 'wb') as image_to_save:
            decoded_tree_data = base64.decodebytes(thetree_bytes)
            image_to_save.write(decoded_tree_data)
